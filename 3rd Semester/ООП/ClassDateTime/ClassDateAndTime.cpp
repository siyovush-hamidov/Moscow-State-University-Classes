...
void DateTime :: Time(char *x)
{
    ...
    MainValue += double(TimeValue) / 86400.0;
}
...
{
    DateTime a;
    MainValue += double(x) / double(86400);
...
string DateTime :: ShowDate()
{
    ...
    int allmonth[12] = {31,28,31,30,31,30,31,31,30,31,30,31}, VariableYear = 365;
...
{
    stringstream x;
    int TimeValue = ((MainValue - floor(MainValue)) * 86400) + 1;
    second = TimeValue % 60;
...
{
    cout << "Fractional To Time: " << "Fractional Part: " << floor(MainValue) << " -> " << ShowTime() << '\n';
}

void DateTime :: TimeToFractional()
{
    cout << "Time To Fractional: " << "Time: " << ShowTime() << " -> " << floor(MainValue) << '\n';
}
...
{   
    DateTime a, b;
    a.DateAndTime("30:02:1908|10:0:0");
